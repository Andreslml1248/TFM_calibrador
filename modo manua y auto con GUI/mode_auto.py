...
class AutoView(ttk.Frame):
    ...
    def _tick(self):
        try:
            ...
            # ✅ LIVE: ahora también DUT (con calibración lineal)
            try:
    ...
    def _dut_text_live(self) -> str:
        ...
        if mode == "A0":
            if bool(getattr(config, "USE_A0_CAL", True)):
                vin = float(getattr(config, "A0_VIN_GAIN", 1.0)) * vadc + float(getattr(config, "A0_VIN_OFFSET", 0.0))
            else:
                vin = vadc
            return f"DUT(A0)= {vin:5.3f} V | Vadc={vadc:5.3f} V"

        if bool(getattr(config, "USE_A1_CAL", True)):
            ima = float(getattr(config, "A1_IMA_GAIN", 1.0)) * vadc + float(getattr(config, "A1_IMA_OFFSET", 0.0))
            return f"DUT(A1)= {ima:6.2f} mA | Vadc={vadc:5.3f} V"

        return f"DUT(A1)= Vadc={vadc:5.3f} V"
